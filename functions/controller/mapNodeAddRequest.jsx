...
async function mapNodeAddRequest(projectId, parentId, query) {
    ...
    if (!projectId || !parentId || !query) {
        log("ERROR", 'Missing parameters @mapNodeAddRequest');
        return;
    }
    ...
    try {
        ...
        if (!map) {
            log("WARNING", "Invalid map for background processing @mapNodeAddRequest:", projectId);
            return {
                statusCode: 500,
        ...
        if (map.creationStatus !== 'created' && map.creationStatus !== 'failed') {
            log("WARNING", "Map creation status already processing @mapNodeAddRequest:", projectId);
            return {
                statusCode: 400,
                body: JSON.stringify({ error: 'Unable to rewrite node' })
            };
        ...
        log("WARNING", 'Unable to create map @mapNodeAddRequest', error);
        if (map) {
            map.creationStatus = 'failed';
            await mapUpdate(map);
        }
