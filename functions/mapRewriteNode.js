...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!userId || userId.trim().length === 0 ||
      ...
      log("WARNING", 'Invalid body @mapRewriteNode', JSON.stringify(body));
      return {
        statusCode: 400,
    ...
    if (!token || token.trim().length === 0) {
      log("WARNING", 'Missing token @mapRewriteNode');
      return {
        statusCode: 401,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    ...
    if (typeof userId !== 'string' || userId.length > 50 ||
        ...
        typeof query !== 'string' || query.length > 500) {
            log("WARNING", 'Request blocked by anti-malicious check @mapRewriteNode');
            return {
                statusCode: 400,
                headers: { ...corsHeaders, 'Content-Type': 'application/json' },
                body: JSON.stringify({ error: 'Invalid request' })
    ...
    if (!updatedMap) {
      log("ERROR", 'Unable to rewrite node @mapRewriteNode');
      return {
        statusCode: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  ...
  // Catch error
    ...
    log("ERROR", `Error in mapRewriteNode endpoint: ${error.message}`);
    return {
      statusCode: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
