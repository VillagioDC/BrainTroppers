...
const mapNodeRewrite = require('./controller/mapNodeRewrite.jsx');
const log = require('./utils/log.jsx');

/* PARAMETERS
    input {headers: {Authorization: Bearer <token>}, body: {projectId, nodeId, query}} - API call
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { projectId, nodeId, query } = parsedBody;
    ...
    const updatedNode = await mapNodeRewrite(map, nodeId, query);
    if (!updatedNode) {
      log('SERVER ERROR', 'Unable to rewrite node');
      return {
        statusCode: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
