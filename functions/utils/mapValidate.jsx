...
function mapValidate(map) {
    ...
    if (!map || typeof map !== 'object' || Array.isArray(map)) {
        log("SERVER WARNING", "Invalid map @mapValidate", `map: ${typeof map}`);
        return false;
    }
    ...
    if (actualFields.some(field => !expectedFields.includes(field))) {
        log("SERVER WARNING", "Invalid fields @mapValidate", `map: ${JSON.stringify(map)}`);
        return false; 
    }

    // Validate projectId
    if (!map.projectId || typeof map.projectId !== 'string' || map.projectId.length > 50 ) {
        log("SERVER WARNING", "Invalid projectId @mapValidate", `map: ${JSON.stringify(map)}`);
        return false;
    }
    ...
    if (!map.owner || typeof map.owner !== 'string' || map.owner.length > 50 ) {
        log("SERVER WARNING", "Invalid owner @mapValidate", `map: ${JSON.stringify(map)}`);
        return false;
    }

    // Validate colabs (array, can be empty)
    if (!Array.isArray(map.colabs)) {
        log("SERVER WARNING", "Invalid colabs @mapValidate", `map: ${JSON.stringify(map)}`);
        return false;
    }
    for (const colab of map.colabs) {
        if (!colab || typeof colab !== 'object' || Array.isArray(colab)) {
            log("SERVER WARNING", "Invalid colab type @mapValidate", `colab: ${JSON.stringify(colab)}`);
            return false;
        }
        // At least one of email or userId must be present
        if (!colab.email && !colab.userId) {
            log("SERVER WARNING", "Invalid colab (no colabs) @mapValidate", `colab: ${JSON.stringify(colab)}`);
            return false;
        }
        ...
        if (colab.email && (typeof colab.email !== 'string' || colab.email.length > 100 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(colab.email))) {
            log("SERVER WARNING", "Invalid colab email @mapValidate", `colab: ${JSON.stringify(colab)}`);
            return false;
        }
        ...
        if (colab.userId && (typeof colab.userId !== 'string' || colab.userId.length > 50 || /[<>{};]/.test(colab.userId))) {
            log("SERVER WARNING", "Invalid colab userId @mapValidate", `colab: ${JSON.stringify(colab)}`);
            return false;
        }
        ...
        if (Object.keys(colab).some(field => !validColabFields.includes(field))) {
            log("SERVER WARNING", "Invalid colab fields @mapValidate", `colab: ${JSON.stringify(colab)}`);
            return false;
        }
    }
    ...
    if (!map.userPrompt || typeof map.userPrompt !== 'string' || map.userPrompt.length > 1000 || /[<>{};]/.test(map.userPrompt)) {
        log("SERVER WARNING", "Invalid userPrompt @mapValidate", `map: ${JSON.stringify(map)}`);
        return false;
    }
    ...
    if (!map.title || typeof map.title !== 'string' || map.title.length > 200 || /[<>{};]/.test(map.title)) {
        log("SERVER WARNING", "Invalid title @mapValidate", `map: ${JSON.stringify(map)}`);
        return false;
    }
    ...
    if (!map.lastUpdated || !(map.lastUpdated instanceof Date || (typeof map.lastUpdated === 'string' && !isNaN(Date.parse(map.lastUpdated))))) {
        log("SERVER WARNING", "Invalid lastUpdated @mapValidate", `map: ${JSON.stringify(map)}`);
        return false;
    }

    // Validate nodes (array, can be empty)
    if (!Array.isArray(map.nodes)) {
        log("SERVER WARNING", "Invalid nodes @mapValidate", `map: ${JSON.stringify(map)}`);
        return false;
    }
    ...
    for (const node of map.nodes) {
        if (!node || typeof node !== 'object' || Array.isArray(node)) {
            log("SERVER WARNING", "Invalid node type @mapValidate", `node: ${JSON.stringify(node)}`);
            return false;
        }
        ...
        if (Object.keys(node).some(field => !validNodeFields.includes(field))) {
            log("SERVER WARNING", "Invalid node fields @mapValidate", `node: ${JSON.stringify(node)}`);
            return false;
        }
        ...
        if (!node.nodeId || typeof node.nodeId !== 'string' || node.nodeId.length > 50 || /[<>{};]/.test(node.nodeId)) {
            log("SERVER WARNING", "Invalid nodeId @mapValidate", `node: ${JSON.stringify(node)}`);
            return false;
        }
        ...
        if (!node.shortName || typeof node.shortName !== 'string' || node.shortName.length > 50) {
            log("SERVER WARNING", "Invalid shortName @mapValidate", `node: ${JSON.stringify(node)}`);
            return false;
        }
        // Validate content (non-empty string, max 200 chars, no malicious content)
        if (!node.content || typeof node.content !== 'string') {
            log("SERVER WARNING", "Invalid content @mapValidate", `node: ${JSON.stringify(node)}`);
            return false;
        }
        ...
        if (!node.colorScheme || typeof node.colorScheme !== 'string' || node.colorScheme.length > 20 || !/^#?[A-Za-z0-9]+$/.test(node.colorScheme)) {
            log("SERVER WARNING", "Invalid colorScheme @mapValidate", `node: ${JSON.stringify(node)}`);
            return false;
        }
        // Validate layer (non-negative integer)
        if (typeof node.layer !== 'number' || !Number.isInteger(node.layer) || node.layer < 0) {
            log("SERVER WARNING", "Invalid layer @mapValidate", `node: ${JSON.stringify(node)}`);
            return false;
        }
    }
