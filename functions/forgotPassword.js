...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!credentials || 
      ...
      log("WARNING", 'Invalid credentials @forgotPassword', JSON.stringify(body));
      return {
        statusCode: 400,
    ...
    if (typeof credentials.email !== 'string' || credentials.email.length > 50) {
            log("WARNING", 'Request blocked by anti-malicious check @forgotPassword', JSON.stringify(body));
            return {
                statusCode: 400,
                headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    ...
    // Check response
    if (!response) {
      log("ERROR", 'Failed sending password reset email @forgotPassword', JSON.stringify(response));
      return {
        statusCode: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  ...
  // Catch error
    ...
    log("ERROR", `Error in forgotPassword endpoint: ${error.message}`);
    return {
      statusCode: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
